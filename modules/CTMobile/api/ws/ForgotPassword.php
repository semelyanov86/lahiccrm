	...
	function process(CTMobile_API_Request $request) {
		...
		if (vtlib_purify($request->get('emailId')) == $email) {
			...
			if($system_Count != 0){
				...
				if ($status === 1){
				   $statusMessage = vtranslate('Mail send successfully','CTMobile');
				   $result = array('code' => 1,'message' => $statusMessage);
				   $response->setResult($result);
				}else{
				   $statusMessage = vtranslate('Mail not sent to Client','CTMobile');
				   $result = array('code' => 0,'message' => $statusMessage);
				   $response->setError(0,$statusMessage);
				}
			}else{
				 $statusMessage = vtranslate('Outgoing server is not enabled, please configure from CRM','CTMobile');
				 $result = array('code' => 0,'message' => $statusMessage);
				 $response->setError(0,$statusMessage);
			}
		}else {
			$statusMessage = vtranslate('Email Id or username not match with your record','CTMobile');
			$result = array('code' => 0, 'message' => $statusMessage);
			$response->setError(0,$statusMessage);
		}
