...
class CTMobile_WS_SendFeedback extends CTMobile_WS_Controller {
	...
	function process(CTMobile_API_Request $request) {
		...
		if(!empty($user_id) && !empty($message)){
			...
			if($mail == 1){
				$message = vtranslate('Mail Sent','CTMobile');
				$response->setResult(array('code'=>'1','message'=>$message));
			}else{
				$message = vtranslate('Mail Not Sent','CTMobile');
				$response->setResult(array('code'=>'0','message'=>$message));
			}				 
			return $response;
		}else{
			$message = vtranslate('Your feedback is valuable for us - it cannot be empty','CTMobile');
