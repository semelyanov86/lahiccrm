(function () {
    ...
    directives.directive('selectreadonly', function ($timeout, $parse) {
        return {
            restrict: 'AC',
            require: 'ngModel',
            link: function (scope, element, attributes) {
                    ...
                    scope.$watchCollection(strObj, function (newVal, oldVal) {
                        // if (newVal !== oldVal) {
                            ...
                            for (var i = 0; i < options.length; i++) {
                                objOption = $(options[i]);
                                var textOption = objOption.text();
                                for (var j = 0; j < allItems.length; j++) {
                                    if(textOption == allItems[j].label){
                                        objOption.attr("data-info", JSON.stringify(allItems[j]));
                                        break;
                                    }
                                }
                            }
