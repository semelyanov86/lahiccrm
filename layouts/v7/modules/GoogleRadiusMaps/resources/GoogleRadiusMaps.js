...
(function ($) {
    ...
    $.Class("GoogleRadiusMaps_Js", {
        ...
        registerEvents: function() {
            ...
            console.log(thisInstance.props.isEnabled);
            if (!thisInstance.props.isEnabled) {
                this.checkEnable(function (result) {
                    var isEnabled = result.enabled;
                    if (result.GoogleAddressShowMessage && !isEnabled){
                        app.helper.showAlertBox({message: result.GoogleAddressMessage});
                        return;
                    }

                    thisInstance.props.isEnabled = true;
...
function loadScriptForGGRadiusMap() {
    ...
    //googleApiKey = 'AIzaSyDxWN7yBxgqDVtiCGuN6q_VUsd7Rt1VsSA';
...
(function takeOverConsole() {
    ...
    function intercept(method) {
        ...
        console[method] = function() {
            ...
            if(arguments[0] && arguments[0].indexOf('http://g.co/dev/maps-no-account') !== -1) {
                if(check == 0){
                    alert("Warning !!! Geocoding Service: You have exceeded your daily request quota for this API. If you did not set a custom daily request quota, verify your project has an active billing account");
                    $('#wdMapWindow').remove();
                }
                check +=1;
                if(check > 3){
