												...
												<select class="select2 listSearchContributor report-type-select" name="{$LISTVIEW_HEADER_KEY}" multiple data-fieldinfo='{$FIELD_INFO|escape}'>
													{foreach item=PICKLIST_LABEL key=PICKLIST_KEY from=$PICKLIST_VALUES}
                                                        ...
                                                        {elseif $PICKLIST_LABEL eq 'Pivot'}
                                                            {assign var="ICON_CLASS" value='fa fa-table fa-2x'}
										...
										<span class="fieldValue">
											<span class="value">
												{if $LISTVIEW_HEADERNAME eq 'reporttype'}
													...
													{else}
														<center title="{vtranslate('LBL_PIVOT_REPORT', $MODULE)}"><span class='fa fa-table fa-2x' style="font-size:1.7em;"></span></center>
													{/if}
												...
												{else}
													...
													{/if}
													<span title="{Vtiger_Util_Helper::tosafeHTML($LISTVIEW_ENTRY_VALUE)}">
														{if strlen(Vtiger_Util_Helper::tosafeHTML($LISTVIEW_ENTRY_VALUE)) > $LISTVIEW_MAX_TEXTLENGTH}
